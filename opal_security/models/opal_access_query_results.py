...
class OpalAccessQueryResults(BaseModel):
    """
    Paginated results of an ACCESS-type OpalQuery — one edge per matched (principal, entitlement, access level) grant.
    """ # noqa: E501
    type: StrictStr
    edges: List[OpalAccessResultEdge] = Field(description="List of matched access grants.")
    page_info: PageInfo = Field(alias="pageInfo")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "edges", "pageInfo"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACCESS']):
            raise ValueError("must be one of enum values ('ACCESS')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "edges": [OpalAccessResultEdge.from_dict(_item) for _item in obj["edges"]] if obj.get("edges") is not None else None,
