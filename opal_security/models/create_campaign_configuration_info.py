...
class CreateCampaignConfigurationInfo(BaseModel):
    ...
    """ # noqa: E501
    query: OpalAccessPathQueryBody = Field(description="Access-path query defining the scope of access to review. Required. Uses the same principalFilter / entitlementFilter shape as ACCESS OpalQuery. Must include at least one of principalFilter or entitlementFilter.  Campaign scope only supports direct access edges: `edgeFilter.directOnly` defaults to `true`, is always stored as `true`, and passing `false` returns 400. ")
